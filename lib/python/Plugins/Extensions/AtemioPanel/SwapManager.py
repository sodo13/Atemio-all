...
config.atemio.swapautostart = ConfigYesNo(default=False)
...
def SwapAutostart(reason, session=None, **kwargs):
	global startswap
	if reason == 0:
		if config.atemio.swapautostart.value:
...
class AtemioSwapPanel(Screen):
	skin = """
	  ...
	  <ePixmap alphatest="blend" pixmap="skin_default/buttons/blue.png" position="1000,637" size="30,35" />
	  ...
	  <widget font="Regular;22" halign="left" name="key_blue" position="1035,640" size="160,24" transparent="1" zPosition="1" />
	  <widget alphatest="on" name="autostart_off" pixmap="skin_default/icons/lock_off.png" position="470,193" size="25,25" transparent="1" zPosition="1" />
	  <widget alphatest="on" name="autostart_on" pixmap="skin_default/icons/lock_on.png" position="470,193" size="25,25" transparent="1" zPosition="2" />
	  <widget font="Regular;22" name="lab1" position="500,180" size="480,45" transparent="1" valign="center" />
	  <widget font="Regular;20" name="lab2" position="470,233" size="150,30" transparent="0" valign="center" />
	  <widget backgroundColor="#5678a4" font="Regular;20" name="labplace" position="620,233" size="220,30" valign="center" />
	  <widget font="Regular;20" name="lab3" position="470,263" size="150,30" transparent="0" valign="center" />
	  <widget backgroundColor="metrixBackground" font="Regular;20" name="labsize" position="620,263" size="220,30" valign="center" />
	  <widget font="Regular;20" name="lab4" position="470,293" size="150,30" transparent="0" valign="center" />
	  <widget backgroundColor="red" font="Regular;20" halign="center" name="inactive" position="620,293" size="100,30" valign="center" />
	  <widget backgroundColor="green" font="Regular;20" halign="center" name="active" position="620,293" size="100,30" valign="center" zPosition="1" />
	</screen>
	"""
	...
	def getSwapDevice(self):
		...
		if path.exists('/etc/rcS.d/S98SwapManager'):
			...
			config.atemio.swapautostart.value = True
			config.atemio.swapautostart.save()
	...
	def updateSwap2(self, result=None, retval=None, extra_args=None):
		...
		if config.atemio.swapautostart.value and self.swap_place:
			self['autostart_off'].hide()
			self['autostart_on'].show()
		else:
			config.atemio.swapautostart.setValue(False)
			config.atemio.swapautostart.save()
			configfile.save()
			self['autostart_on'].hide()
			self['autostart_off'].show()
	...
	def doCSplace(self, name):
		if name:
			...
			myoptions = [[_("8 Mb"), '8192'], [_("16 Mb"), '16384'], [_("32 Mb"), '32768'], [_("64 Mb"), '65536'], [_("96 Mb"), '98304'], [_("128 Mb"), '131072'], [_("256 Mb"), '262144'], [_('512 Mega'), '524288'], [_('1 Giga'), '1048576']]
	...
	def autoSsWap(self):
		if self.swap_place:
			if config.atemio.swapautostart.value:
				...
				config.atemio.swapautostart.setValue(True)
				config.atemio.swapautostart.save()
			configfile.save()
		else:
